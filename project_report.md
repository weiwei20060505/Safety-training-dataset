...
2. **`01_Metrics_Trends_split_y`**：依 $y_1$ 分流 (iso_0, iso_1) 之 Brier Score 與 Log Loss 隨層數變化趨勢圖。
3. **`02_Reliability_Curves_split_y`**：依 $y_1$ 分流 (iso_0, iso_1) 之可靠度對比曲線。
4. **`02_Reliability_Curves_combined`**：合併 $y_1 == 0$ 與 $y_1 == 1$ 校正前後之可靠度對比曲線（紅/藍與虛/實線對比）。
5. **`03_Quadrant_Histograms`**：2x2 全量四象限預測置信度直方圖。
6. **`04_Score_Histograms`**：依 $y_1$ 分流 (iso_0, iso_1) 之正負樣本預測分數直方圖。
7. **`05_Brier_Components`**：依 $y_1$ 分流 (iso_0, iso_1) 之 Brier 組分雙 Y 軸圖（左軸 Rel/Res 柱狀圖，右軸 Weight 折線圖標明「樣本比例」）。
8. **`06_Step_Mappings`**：依 $y_1$ 分流 (iso_0, iso_1) 之保序校正分數映射階梯圖。
