...
##### **3. MLPClassifier (MLP) 核心參數設定 (全場冠軍🏆)：**
...
##### **通用訓練分佈與重現性設定：**
- `RandomUnderSampler`：**完全移除**（保留 78,000 筆真實自然分佈，維護 $P(Y=1)$ 先驗機率與 Exchangeability）
...
- **第二階段（機率校正與終極精篩）**：
  保留候選探針，於獨立 `test1` (2,000筆) 上執行 Isotonic/Platt 雙軌機率校正，並於 `test2` (3,000筆) 上以 ECE (Expected Calibration Error) 與校正後 AUC 選出最終分類器。
...
---
### 🏆 第一階段階段性成果 (LGBM 78k 數據集表現)

針對提示詞有害性任務 (Y2)，全量數據 + 1000 輪早停顯著提升探針表現：

| 特徵層數 | 最佳輪數 (`best_iteration`) | 驗證集 Log Loss | AUC 曲線下面積 | 驗證集 Accuracy |
| :--- | :--- | :--- | :--- | :--- |
| **Layer 1** | 300 輪 (早停) | 0.5384 | 0.8009 | 71.75% |
| **Layer 2** | 417 輪 (早停) | 0.4990 | 0.8361 | 75.05% |
| **Layer 3** | 393 輪 (早停) | 0.3998 | 0.9071 | 82.35% |
| **Layer 4** | 453 輪 (早停) | 0.3829 | 0.9141 | 82.75% |
| **Layer 5** | **706 輪 (早停)** | **0.3749 (歷史新低)** | **0.9149** | **82.95%** |
| **Layer 6** | 510 輪 (早停) | 0.3890 | 0.9103 | 82.50% |

> **結論**：Layer 4 與 Layer 5 為黃金特徵層，AUC 穩定突破 **0.914+**，Log Loss 大幅下降 18.2%。
...
![Y2 雙軌機率校正 2x4 大圖](results/v1_baseline/plots/02_Stage2_Calibration_Curves/07_Joint_Calibration/big_grid/y2_joint_calibration_2x4_grid.png)
...
##### **淘汰與選拔決策 (Selection Decision)：**
1. **明確剔除 Layer 1 & 2**：前兩層 AUC (0.79~0.86) 與 Brier 顯著偏高，屬未解耦之雜訊層。
2. **精選二階段金牌矩陣 (12 個組合)**：
   - **模型**：`MLP` (最強非線性)、`LR` (對照基準 Anchor)、`LGB` (優異樹模型)
   - **層數**：`Layer 3`、`Layer 4`、`Layer 5`、`Layer 6` (AUC 0.92~0.9635)
